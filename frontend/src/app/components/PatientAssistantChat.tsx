        ...
        <Card className="fixed bottom-6 right-6 w-80 md:w-96 h-[500px] flex flex-col bg-white border-slate-200 shadow-2xl z-50 animate-in slide-in-from-bottom-5">
          ...
          <CardContent className="p-0 flex-1 flex flex-col overflow-hidden">
            ...
            <div className="p-3 bg-white border-t border-slate-200">
              {file && (
                <div className="flex items-center gap-2 mb-2 text-xs text-cyan-700 bg-cyan-50 p-2 rounded-md border border-cyan-100">
                  <Paperclip className="w-3 h-3" />
                  {file.name}
                  <button onClick={() => setFile(null)} className="ml-auto hover:text-slate-900" type="button"><X className="w-3 h-3" /></button>
                </div>
              )}
              <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="flex items-center gap-2">
                ...
                />
                <Button type="button" variant="ghost" size="icon" className="h-9 w-9 text-slate-500 hover:text-slate-900" onClick={() => document.getElementById('chat-file-upload')?.click()}>
